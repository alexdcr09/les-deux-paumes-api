    ...
    Video.findById(req.params.videoId, (err, data) => {
        if (err) {
            if (err.kind === "not_found") {
                ...
                res.status(500).send({
                    message:
                        "Error retrieving Video with id " + req.params.videoId,
                });
            }
        } else res.send(data);
    });
    ...
    Video.remove(req.params.videoId, (err, data) => {
        if (err) {
            if (err.kind === "not_found") {
                ...
                res.status(500).send({
                    message:
                        "Could not delete video with id " + req.params.videoId,
                });
            }
